...
**Discogs stale cache:** In **Settings**, you can queue a background job that resets stored Discogs-detail schema versions for **your** releases (and artists and labels linked to those releases), then re-fetches from Discogs so you pick up upstream changes without bumping app constants—large collections may take a long time due to Discogs rate limits.
